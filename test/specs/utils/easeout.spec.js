...
describe('utils::easeout', function() {
  it('should `undefined` when `oldPos === newPos`', function() {
    expect(easeout(0, 0, 0)).to.equal(undefined);
  });

  it('should a number when `rate` not a number', function() {
    expect(easeout(0, 10, '123')).to.equal(2);
  });

  it('should a number when `oldPos` not a number', function() {
    expect(easeout('123', 10, 5)).to.equal(2);
  });

  it('should a number when `newPos` not a number', function() {
